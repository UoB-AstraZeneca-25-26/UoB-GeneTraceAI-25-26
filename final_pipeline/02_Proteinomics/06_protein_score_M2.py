...
def robust_z_matrix(X: np.ndarray, min_peers: int = MIN_PEERS) -> np.ndarray:
    ...
    scale = np.where(np.isclose(mad, 0.0) | ~np.isfinite(mad), std, 1.4826 * mad)
    scale = np.where(np.isclose(scale, 0.0) | ~np.isfinite(scale), 1.0, scale)
    Z = (X - med) / scale
