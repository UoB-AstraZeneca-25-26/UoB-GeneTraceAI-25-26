...
# PROCAN_SOURCE was "protein_matrix_averaged_20250211" -- that table does not
# exist in the live warehouse. Confirmed (2026-08-24) to be the same dataset
# as the live `procan_proteomics` table: 8,453/8,453 protein columns match
# exactly, 948/948 cell lines resolve 1:1 via sanger_model_id, and sampled
# (protein, cell_line) values are byte-identical between the raw source file
# (data/proteomics_procan/Protein_matrix_averaged_20250211.tsv) and
# procan_proteomics -- procan_proteomics is that file after SIDM->ACH model_id
# resolution and header promotion, not a reprocessing that changed values.
PROCAN_SOURCE = "procan_proteomics"
...
# gene_roster does not exist in the live warehouse either -- it combined
# uniprot_id(s)/procan_uniprot_id/gene_name(s) as multi-value columns that are
# split live across `gene`, `procan_protein_map`, and `protein_map`. Confirmed
# (2026-08-24) this reconstruction is clean: 0 duplicate uniprot_id rows in
# either map table, 96.7-96.8% resolve to a gene_id (the unresolved remainder
# is real symbol/alias mismatch, not a join artifact), and the resulting
# ProCAN gene coverage (8,186 genes) matches protein_scorer.py's own
# already-live, already-validated resolution exactly -- this mirrors that
# proven approach rather than introducing a new one.
...
def build_uniprot_to_ensg(con) -> dict[str, str]:
    ...
    for table in ["protein_map", "procan_protein_map"]:  # ProCAN last -> wins on collision
        df = con.execute(f'SELECT uniprot_id, gene_symbol FROM {table}').df().dropna()
        for uniprot, symbol in zip(df["uniprot_id"].str.lower(), df["gene_symbol"].str.lower()):
            ensg = sym_map.get(symbol)
            if ensg:
                mapping[uniprot] = ensg
    return mapping


def build_symbol_to_ensg(con) -> dict[str, str]:
    gene_df = con.execute("SELECT gene_id, hugo_symbol FROM gene").df().dropna()
    return dict(zip(gene_df["hugo_symbol"].str.lower(), gene_df["gene_id"].str.upper()))
...
# ---------------------------------------------------------------------
# Lineage-conditioned robust z
# ---------------------------------------------------------------------

# Minimum non-NaN lines per (source, lineage) stratum to trust a z-score.
# Mirrors common.py's MIN_PEERS=5 and transcriptomics.py's nb<5 fallback --
# M2 had no such gate, which combined with plain (non-NaN-aware) np.median/
# np.std let severely under-observed columns produce a real-but-spurious
# median/scale instead of NaN, driving the extreme outlier z-scores flagged
# earlier (min ~-14378, max ~80725). Added 2026-08-25.
MIN_PEERS = 5

# ── Per-(gene, lineage) scale shrinkage (NEW in v2) ─────────────────────────
# The MIN_PEERS gate above stops genes with TOO FEW lines from being scored
# at all, but says nothing about genes that clear MIN_PEERS with a small,
# unstable lineage (e.g. n=19, as traced for ARHGEF39/ach-000727: median/MAD
# computed from 19 points gave z=-5.98 for a real, non-corrupted -2.33
# reading, vs. z=-2.87 if scored against the whole panel's spread instead --
# MAD's own robustness to outliers is what lets a genuine outlier look this
# extreme against a small, tightly-clustered peer group). This is additive to
# MIN_PEERS, not a replacement: the gate below still NaNs out anything under
# MIN_PEERS regardless of shrinkage; shrinkage only tempers the scale for
# genes that pass the gate but are still judged against a small group.
#
# Reference/parameters match the standalone test that validated this
# approach (test_shrinkage_fix.py, 15 known-outlier genes, checked directly
# rather than assumed): the "wider reference" is each gene's OWN-PLATFORM,
# ALL-LINEAGES-POOLED MAD (not cross-platform, matching that zscore_platform()
# is already called once per platform), computed from the same raw values,
# not from an intermediate z. n0=25 reuses the same convention already used
# twice elsewhere in this project (RNA stratum shrinkage in transcriptomics.py,
# the core_score.py protein-residual fix) -- not independently re-derived for
# this stage; flagged as a first-pass choice, same as those two.
SHRINK_N0 = 25


def robust_z_matrix(X: np.ndarray, global_scale: np.ndarray | None = None,
                    min_peers: int = MIN_PEERS, n0: int = SHRINK_N0) -> np.ndarray:
    """global_scale: optional (n_genes,) array of each column's panel-wide
    scale, aligned to X's columns. When given, each column's local scale is
    shrunk toward it with weight w = n_valid/(n_valid+n0) -- more shrinkage
    for genes with fewer valid lines in this lineage. When omitted, behaves
    exactly as before (used where no panel-wide reference is available/
    meaningful, so the function's existing callers are unaffected by default).
    """
    ...
    scale_local = np.where(np.isclose(mad, 0.0) | ~np.isfinite(mad), std, 1.4826 * mad)
    scale_local = np.where(np.isclose(scale_local, 0.0) | ~np.isfinite(scale_local), 1.0, scale_local)

    if global_scale is not None:
        gs = np.asarray(global_scale, dtype=float).reshape(1, -1)
        gs = np.where(np.isclose(gs, 0.0) | ~np.isfinite(gs), 1.0, gs)
        w = (n_valid / (n_valid + n0)).reshape(1, -1)
        scale = np.sqrt(w * scale_local ** 2 + (1.0 - w) * gs ** 2)
    else:
        scale = scale_local

    Z = (X - med) / scale
    Z[:, n_valid < min_peers] = np.nan   # MIN_PEERS gate unchanged, still fires regardless of shrinkage
    return Z
...
def zscore_platform(df, lineage_map, source_name) -> pd.DataFrame:
    ...
    # Panel-wide (all lineages, this platform) reference scale per gene,
    # computed ONCE before the per-lineage loop -- the "wider reference" each
    # lineage's own scale is shrunk toward. Matches the standalone test:
    # global MAD from raw values, per (gene, platform), not per (gene, lineage).
    gene_cols_all = [c for c in wide.columns if c != "lineage"]
    with np.errstate(all="ignore"):
        Xall = wide[gene_cols_all].to_numpy(dtype=float)
        med_all = np.nanmedian(Xall, axis=0, keepdims=True)
        mad_all = np.nanmedian(np.abs(Xall - med_all), axis=0, keepdims=True)
    global_scale_all = dict(zip(gene_cols_all, (1.4826 * mad_all).ravel()))
    ...
    for lineage, grp in wide.groupby("lineage", sort=True):
        ...
        if not gene_cols:
            continue
        gscale = np.array([global_scale_all[g] for g in gene_cols])
        Z = robust_z_matrix(grp[gene_cols].to_numpy(dtype=float), global_scale=gscale)
...
def compute_from_sources(raw_longs, lineage_map, thr) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Only applied when a row actually combines 2 sources -- a single-source row
    # has no second platform to disagree with, so the gene-level confidence
    # (derived from OTHER cell lines where both platforms were present) does not
    # apply to it. n_sources == 1 rows pass through as z_raw unmodified.
