...
# v4 TEST HARNESS: isolates "Stage 2 shrinkage fix ALONE" -- unmodified
# core_score.py combination logic (no protein-residual shrinkage/clip here),
# but reads bulk_prot_z_v2.parquet (06_protein_score_M2_v2.py's Stage-2-shrunk
# output) instead of the live bulk_prot_z.parquet. Writes to separate files.
PROT_Z_V2     = PROT_Z.with_name("bulk_prot_z_v2.parquet")
CORE_SCORE_V4 = CORE_SCORE.with_name("core_score_v4.parquet")
GENE_DISP_V4  = GENE_DISP.with_name("gene_dispersion_v4.parquet")
...
def run():
    ...
    core_long.to_parquet(CORE_SCORE_V4, index=False)
    disp.to_parquet(GENE_DISP_V4, index=False)

    elapsed = time.time() - t0
    print(f"\nDone in {elapsed:.0f}s")
    print(f"core_score (v4, Stage-2-fix ONLY): {len(core_long):,} rows  ->  {CORE_SCORE_V4}")
