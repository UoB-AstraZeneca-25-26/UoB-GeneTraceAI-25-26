...
def run():
    ...
    core_long.to_parquet(CORE_SCORE_V3, index=False)
    disp.to_parquet(GENE_DISP_V3, index=False)

    elapsed = time.time() - t0
    print(f"\nDone in {elapsed:.0f}s")
    print(f"core_score (v3, NOT the live output): {len(core_long):,} rows  ->  {CORE_SCORE_V3}")
