"""
Scoring/core_score_variant_C_no_residualization.py  (TEST HARNESS -- not the
live scorer, not wired into run_all.py. Part of the three-way residualization
comparison: see core_score_variant_A_current.py and
core_score_variant_B_flipped.py.)
-----------------------------------------------------------------------------
No cross-adjustment in either direction. Both arms enter as their own
independently standardized z-scores -- the plain Stouffer combination of two
standardized inputs, with no correction for the fact that RNA and protein
are correlated, non-independent measurements of related biology.
...
OUT = CORE_SCORE.with_name("core_score_variant_C.parquet")
