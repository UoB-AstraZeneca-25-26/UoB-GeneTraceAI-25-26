"""
Scoring/core_score_v8_beta_shrinkage.py  (CANDIDATE -- builds on the live,
C7-fixed core_score.py, unchanged except for ONE targeted addition, finding
"C3" from the full mathematical audit.)
-----------------------------------------------------------------------------
C3 candidate: shrink sd_rna/sd_prot before forming beta_g.

Live core_score.py computes beta_g = rho_g * (sd_prot_raw/sd_rna_raw), where
rho_g is carefully empirical-Bayes shrunk (lam=K_RHO/(n+K_RHO)) but sd_rna_raw
and sd_prot_raw are RAW, unshrunk per-gene estimates -- even for genes barely
above N_MIN_RHO=30, where a single-gene SD estimate is itself noisy. This is
...
This candidate applies the IDENTICAL shrinkage shape already used elsewhere
in this file (w = n/(n+n0), blend toward the panel-wide SD) to sd_rna and
...
(the same _sd_rna_global/_sd_prot_global already computed in the live file
...
Isolated finding (why-reference task, reproduced against this real
implementation in the verification step): near N_MIN_RHO=30 (30-60 shared
lines), beta_g shifts by median 19.9%, mean 72.1%, p99 1074% when this
shrinkage is applied; far from the threshold (n>200), shift is <1%.

Downstream full-pipeline spike effect: see verification log / WHY_REFERENCE.md
for the actual measured result -- NOT assumed to help just because the
isolated beta_g shift is well-motivated, per this project's now-established
pattern of upstream improvements not always composing well downstream.
...
Writes to:   final_pipeline/outputs/{core_score_v8_beta_shrinkage, gene_dispersion_v8_beta_shrinkage}.parquet
-----------------------------------------------------------------------------
...
OUT_SCORE = CORE_SCORE.with_name("core_score_v8_beta_shrinkage.parquet")
OUT_DISP  = GENE_DISP.with_name("gene_dispersion_v8_beta_shrinkage.parquet")
