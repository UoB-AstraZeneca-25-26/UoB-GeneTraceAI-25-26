...
def collapse_replicates(ax: MethodAxis, X: np.ndarray) -> np.ndarray:
    ...
    """
    if len(ax.models) == len(ax.raw_ids) and np.array_equal(ax.model_ids, ax.models):
        return X
...
def method_z(V: np.ndarray, lineage: np.ndarray, floor: float, p: Params,
    ...
    """
    # Absolute scale floor for near-zero-variance (gene, lineage) pairs --
    # e.g. a globally sparse/mostly-undetected gene (global MAD == 0) paired
    # with a singleton lineage (nb == 1, so lineage MAD == 0 trivially): a
    # real, nonzero numerator then divides by EPS=1e-9 and explodes to
    # billions. common.py's MAD_FLOOR is the right floor to reuse here, not
    # a fresh constant -- both files score the same log2(TPM+1)-scale RNA
    # data, and common.py's own comment says it was "calibrated against the
    # shared transcriptomics notebook", i.e. the same analysis this file
    # implements. Imported lazily (not at module load) so this file's own
    # "no side effects at import" guarantee holds -- common.py pulls in
    # config.py, which creates output directories as an import side effect.
    from utils.common import MAD_FLOOR, MIN_PEERS
    ...
    for _, idx in groups:
        ...
        scale = np.maximum(scale, np.maximum(0.1 * gl, MAD_FLOOR))    # MAD floor
        use_lineage_centre = np.isfinite(med_L) & (nb >= MIN_PEERS)
        ...
        if nb < MIN_PEERS:
            # Too few lines to trust ANY lineage-level statistic, even one
            # blended toward the global scale/median at low weight -- MAD_FLOOR
            # bounds the blowup but a genuinely large numerator (real elevated
            # expression against a near-silent global background) can still
            # clear |z|=10+ from a single-point "lineage". common.py's
            # MIN_PEERS gate and M2's equivalent both mark this no_data rather
            # than fabricate a globally-anchored estimate from one line -- do
            # the same here instead of computing blk/st at all. Scoped to this
            # branch only: nb >= MIN_PEERS groups are completely unaffected.
            blk = np.full(G, np.nan, dtype=np.float32)
            st = np.full(G, 3, np.int8)
        else:
