...
// currentView, so it unmounts on every navigation away (Reference, Results,
// About, ...) and remounts on return -- local state seeded only at mount
// would silently discard any in-progress, not-yet-submitted edits (targets/
// exclusions/topK) on that remount. Lifting the draft itself to the parent
// makes it survive the unmount, the same fix already applied to
// detailCacheRef/profileHistory for the same underlying reason.
export const QueryBuilder: React.FC<QueryBuilderProps> = ({ value, onChange, onSubmit, onOpenReference }) => {
  const { symbols: geneUniverse, isKnown } = useGeneUniverse();
  const { targets, exclusions, topK } = value;
  const [error, setError] = useState<string | null>(null);

  const setTargets = (list: string[]) => onChange({ ...value, targets: list });
  const setExclusions = (list: string[]) => onChange({ ...value, exclusions: list });
  const setTopK = (v: number) => onChange({ ...value, topK: v });
  ...
  const addTarget = (gene: string) => {
    onChange({
      ...value,
      exclusions: exclusions.filter((g) => g !== gene),
      targets: targets.includes(gene) ? targets : [...targets, gene],
    });
  };
  const addExclusion = (gene: string) => {
    onChange({
      ...value,
      targets: targets.filter((g) => g !== gene),
      exclusions: exclusions.includes(gene) ? exclusions : [...exclusions, gene],
    });
  };
  ...
  const handleClear = () => {
    onChange({ targets: [], exclusions: [], lineage: 'Any', topK: 5 });
    setError(null);
  };
