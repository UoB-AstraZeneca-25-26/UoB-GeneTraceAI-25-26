...
/** Cell-line context fields shared across every mode's adapter -- pulled from
 *  the same generic `metadata` block every ranking response already returns. */
export function contextFields(meta?: LineMetadata) {
  return {
    primaryDisease: meta?.primary_disease,
    subtype: meta?.subtype,
    growthPattern: meta?.default_growth_pattern,
    sex: meta?.sex,
    rrid: meta?.rrid,
  };
}
...
// Shared cache key shape for the (gene, model_id, ...query context) detail
// response -- used by both ProfileView and EvidenceMatrix so a line inspected
// in one place is already warm if the other place requests the same pair.
// otherGenes/excludeGenes are folded in (sorted, so argument order doesn't
// fragment the cache) because they change which rna_alternatives come back
// for an otherwise-identical (gene, modelId) pair -- caching on gene+modelId
// alone would let a narrower or wider alternatives list leak between
// different query contexts for the same line.
export const detailCacheKey = (
  ...
  `${gene}::${modelId}::${[...otherGenes].sort().join(',')}::${[...excludeGenes].sort().join(',')}`;

export async function fetchCellLineDetail(
  gene: string,
  modelId: string,
  signal?: AbortSignal,
  cache?: Map<string, CellLineDetailApiResponse>,
  otherGenes: string[] = [],
  excludeGenes: string[] = []
  ...
  const cacheKey = detailCacheKey(g, id, otherGenes, excludeGenes);
  ...
  const params = new URLSearchParams({ gene: g, model_id: id });
  if (otherGenes.length) params.set('other_genes', otherGenes.join(','));
  if (excludeGenes.length) params.set('exclude_genes', excludeGenes.join(','));
  const data = await getJson<CellLineDetailApiResponse>(`${DETAIL_URL}?${params.toString()}`, signal);
