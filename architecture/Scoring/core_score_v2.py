"""
Scoring/core_score_v2.py
-------------------------
Working copy of core_score.py with the RNA-arm standardization fix applied.
Not yet promoted to live -- see the fix investigation report for verification
results and the explicit readiness-to-promote decision point.
...
Architecture:
  ...
  -> core_score_v2.parquet, gene_dispersion_v2.parquet

Combination:
  RNA std z       = (rna_z - median_by_gene_and_nsources) / SD_by_gene_and_nsources,
                    winsorized at +/-RNA_WINSOR_BOUND. Replaces the old
  ...
  core_z = (W_RNA * rna_std_z + W_PROT * prot_resid_z) / sqrt(W_RNA^2 + W_PROT^2)
    -- W_RNA/W_PROT left UNCHANGED in this fix; whether they still make sense
       given the new RNA treatment is an explicitly separate, out-of-scope
...
Writes to:   final_pipeline/outputs/{core_score_v2, gene_dispersion_v2}.parquet
"""
import sys, time
...
# v2 writes to its own paths -- never overwrites the live core_score/gene_dispersion
# outputs. Promoting this fix to live is an explicit, separate decision, not this file.
CORE_SCORE_V2 = CORE_SCORE.with_name("core_score_v2.parquet")
GENE_DISP_V2  = GENE_DISP.with_name("gene_dispersion_v2.parquet")
...
# ── RNA standardization (Candidate 2: per-gene × n_sources) ─────────────────
# Replaces the old within-lineage percentile + clipped norm.ppf treatment.
# That approach double-conditioned RNA (already lineage-conditioned upstream
# in rna_scorer.py/transcriptomics.py) and structurally suppressed RNA
# magnitude relative to protein's uncapped (x-median)/SD treatment.
#
...
# standardization (Candidate 1, matching the protein arm's own treatment
# verbatim) does NOT fix this -- per-n_sources SD stays heterogeneous
...
def run():
    ...
    core_long.to_parquet(CORE_SCORE_V2, index=False)
    disp.to_parquet(GENE_DISP_V2, index=False)

    elapsed = time.time() - t0
    print(f"\nDone in {elapsed:.0f}s")
    print(f"core_score: {len(core_long):,} rows  ->  {CORE_SCORE_V2}")
