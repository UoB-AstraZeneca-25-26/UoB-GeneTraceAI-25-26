"""
Scoring/core_score_v3.py  (WORKING COPY -- not the live scorer, not wired
into run_all.py. See docs/SCORING_METHODS_FULL.md and the protein-shrinkage
cowork task for the investigation this implements.)
----------------------
...
Combination:
  ...
  prot_resid_z    = (prot_resid - med_gene) / SD_final_gene, winsorized at
                    ...
                    (w = n_gene/(n_gene+PROT_RESID_N0)) -- the direct protein
                    analogue of RNA's per-stratum shrinkage, added because the
                    pre-v3 version had NEITHER a shrinkage step NOR a clip
                    (git-diffed against core_score_v1_pre_rna_fix_*.py.bak:
                    the RNA fix added both to the RNA arm and left the protein
                    arm's standardization block byte-for-byte unchanged --
                    an incidental gap, not a considered decision, per the
                    investigation that produced this file). See
                    _standardize_prot_resid_by_gene() for detail, and
                    docs/SCORING_METHODS_FULL.md / the cowork task report for
                    the evidence (a real outlier, ARHGEF39/ach-000727, judged
                    against a 19-line peer group, scored z=-5.98 unshrunk vs.
                    z=-5.17 shrunk -- reduced, not eliminated, because this
                    gene has low variance panel-wide, not just in one lineage).
  ...
  core_z = (W_RNA * rna_std_z + W_PROT * prot_resid_z) / sqrt(W_RNA^2 + W_PROT^2)
    -- W_RNA/W_PROT left UNCHANGED by the RNA standardization fix; whether they
       still make sense given the new RNA treatment is an explicitly separate,
       out-of-scope question for a later task.
  core_score = Phi(core_z)  [monotone relabelling to [0,1]]

  IMPORTANT (set expectations honestly): fixing the protein arm does NOT
  eliminate the core_score=0/1 spikes on its own. Even with a hypothetically
  perfect protein arm (prot_resid_z=0), RNA's own +/-5 clip alone pushes
  core_z past the point where Phi saturates past 0.99/0.01 -- analytically,
  rna_std_z only needs to reach ~3.30 (well under its own +/-5 bound) to do
  this, given W_RNA's ~0.70 fractional share of the combination. Measured on
  the real n_layers=2 population (18.7M RNA rows / 6.76M shared with protein):
  RNA alone (protein held at zero) produces an extreme score on 0.478% of
  rows, against a measured real (both-arms) extreme rate of ~3.10% -- i.e.
  RNA's clip alone already accounts for ~15% of the currently-observed spike,
  and that portion is IRREDUCIBLE by fixing protein. See Part B of the cowork
  task report for the full figures.
...
Writes to:   final_pipeline/outputs/{core_score_v3, gene_dispersion_v3}.parquet
             (NOT core_score.parquet -- this is a working copy; see config
             overrides just below run(), not the shared config.py constants)
"""
import sys, time
...
# v3 is a working copy under investigation -- write to separate files, never
# to the paths the live core_score.py writes, so this cannot clobber
# core_score.parquet / gene_dispersion.parquet no matter how it's invoked.
CORE_SCORE_V3 = CORE_SCORE.with_name("core_score_v3.parquet")
GENE_DISP_V3  = GENE_DISP.with_name("gene_dispersion_v3.parquet")

# v5 TEST HARNESS (extends v3): "BOTH fixes combined" -- v3's protein-residual
# shrinkage+clip (below), reading bulk_prot_z_v2.parquet (Stage-2-shrunk
# input) instead of the live bulk_prot_z.parquet. Separate output files.
PROT_Z_V2     = PROT_Z.with_name("bulk_prot_z_v2.parquet")
CORE_SCORE_V5 = CORE_SCORE.with_name("core_score_v5.parquet")
GENE_DISP_V5  = GENE_DISP.with_name("gene_dispersion_v5.parquet")
...
# ── Protein residual shrinkage + clip (NEW in v3) ───────────────────────────
# core_score_v1_pre_rna_fix_*.py.bak diffed byte-for-byte against the live
# core_score.py: the RNA fix above added shrinkage AND a clip to the RNA arm;
# the protein residual standardization block was not touched at all. No doc
# or code comment anywhere states this was a deliberate asymmetry -- it is an
# inherited gap, the same "fixed one arm, never revisited the other" pattern
# already found once in this project for the RNA percentile step's own
# history. This section closes that gap using the same w=n/(n+n0) shrinkage
# shape as RNA, continuously applied (not gated behind a hard "thin" cutoff
# the way RNA's is) -- a standalone test on 15 known-outlier genes showed
# RNA's own MIN_N=5 hard cutoff would not even fire for the concrete outlier
# case found (n=19 shared lines, above 5), so a continuous blend is used here
# deliberately, not copied verbatim from RNA's exact on/off shape.
#
...
def run():
    ...
    # v3 CHANGE: was a plain (x-median)/SD per gene, with no shrinkage and no
    # bound -- see module docstring and _standardize_prot_resid_by_gene() for
    # why this was a gap, not a design choice, and what closes it.
    ...
    core_long.to_parquet(CORE_SCORE_V5, index=False)
    disp.to_parquet(GENE_DISP_V5, index=False)

    elapsed = time.time() - t0
    print(f"\nDone in {elapsed:.0f}s")
    print(f"core_score (v5, BOTH fixes combined): {len(core_long):,} rows  ->  {CORE_SCORE_V5}")
