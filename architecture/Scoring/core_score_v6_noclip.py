...
Combination:
  ...
  This fix (protein shrinkage+clip) measurably reduces the remainder
  (n_layers=2: 3.098% -> 3.039%; full panel: 2.870% -> 2.847%) but does not
  eliminate it. A follow-up investigation found that shrinking the earlier,
  per-(gene,lineage) z-scoring stage instead (02_Proteinomics/
  06_protein_score_M2.py's robust_z_matrix()) substantially improves the raw
  protein z-scores (|z|>=5 fraction: -68% on the full panel) but, tested end
  to end, made this stage's OWN combined core_score spike slightly WORSE
  (it interacts with the per-gene rho_g/beta_g regression this file fits
  fresh from whatever protein input it receives) -- that fix was evaluated
  and explicitly NOT promoted; see docs/SCORING_METHODS_FULL.md.
...
# v6 TEST HARNESS: identical to live core_score.py EXCEPT both winsor bounds
# below are set to infinity, disabling the +/-5 clip on both arms while
# leaving shrinkage (the actual "normalization" -- blending each estimate
# toward a wider, more stable reference) fully in place. Tests whether the
# clip specifically, on top of shrinkage, is helping or hurting. Writes to
# separate files, never the live core_score.parquet.
CORE_SCORE_V6 = CORE_SCORE.with_name("core_score_v6.parquet")
GENE_DISP_V6  = GENE_DISP.with_name("gene_dispersion_v6.parquet")
...
RNA_WINSOR_BOUND   = np.inf  # v6 TEST: clip disabled (was 5.0 live) -- shrinkage
                         # (above) still applies; only the hard truncation is removed.
...
PROT_RESID_WINSOR_BOUND  = np.inf  # v6 TEST: clip disabled (was 5.0 live).
...
def run():
    ...
    core_long.to_parquet(CORE_SCORE_V6, index=False)
    disp.to_parquet(GENE_DISP_V6, index=False)

    elapsed = time.time() - t0
    print(f"\nDone in {elapsed:.0f}s")
    print(f"core_score (v6, NO CLIP test): {len(core_long):,} rows  ->  {CORE_SCORE_V6}")
