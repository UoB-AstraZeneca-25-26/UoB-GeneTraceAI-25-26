...
# Shrunk 2026-08-25 after the 72-combo widened run proved impractically slow
# in this environment (mid-run rate extrapolated to hours, not minutes).
# floor_q/lower_cut kept at 3 points each -- 0.20 is the known previous edge,
# 0.30/0.50 test whether rho has flattened or is still climbing past it.
# upper_cut fixed at 0.5 (already well-identified). n0 fixed at 25 -- 100 was
# already proven statistically indistinguishable from 25, dropped entirely
# rather than re-tested. Note: lower_cut=0.50 combined with the fixed
# upper_cut=0.50 is filtered out by grid_combinations()'s lower_cut<upper_cut
# rule, so lower_cut effectively only gets tested at 0.20/0.30 here (2 points,
# not 3) -- floor_q is unaffected by that filter and keeps all 3 points.
PARAM_GRID = dict(
    floor_q=[0.20, 0.30, 0.50],
    n0=[25],
    upper_cut=[0.50],
    lower_cut=[0.20, 0.30, 0.50],
)
...
# ----------------------------------------------------------------------
# Fetch memoization -- CACHED-VERSION-ONLY ADDITION, not present in the
# original transcriptomics.py. Disabled (None) by default, so every
# caller other than calibrate_params() behaves identically to the
# original: a real DuckDB fetch every call.
#
# calibrate_params() enables this for its own duration only, because it
# is the one place the exact same (table, gene-list) pair is re-fetched
# unchanged across all 480 grid combinations x 2 holdout passes -- only
# floor_q/n0/upper_cut/lower_cut differ between iterations, never the
# underlying 1200-gene (or 400-gene probe) data. Scoped narrowly so the
# main scoring path (step_score, ~40 gene chunks of a full run) keeps
# its existing no-caching memory behavior -- unbounded caching there
# would grow memory across the whole gene universe, which is out of
# scope for this fix.
# ----------------------------------------------------------------------
_FETCH_CACHE: dict | None = None


def _fetch_cache_enable() -> None:
    global _FETCH_CACHE
    _FETCH_CACHE = {}


def _fetch_cache_disable() -> None:
    global _FETCH_CACHE
    _FETCH_CACHE = None
...
def fetch_wide_chunk(con, table: str, id_col: str, genes: list[str],
    ...
    cache_key = ("wide", table, id_col, tuple(genes))
...
def fetch_hpa_chunk(con, genes: list[str]) -> pd.DataFrame:
    ...
    if not genes:
        ...
        result = pd.DataFrame() if df.empty else df.pivot(index="_id", columns="ensg", values="v")
...
def calibrate_params(con, axes, colmaps, all_genes, models, midx, lineage_s,
    ...
    CACHED-VERSION-ONLY NOTE: wraps the loop body in the fetch-memoization
    scope (_fetch_cache_enable/_disable). Every combination and both
    holdout passes re-request the identical (table, gene-list) data --
    only floor_q/n0/upper_cut/lower_cut change -- so this turns ~960
    redundant DuckDB re-fetches into 1 real fetch per (table, gene-list)
    pair. Nothing between enable/disable is reordered, removed, or
    changed: same calls, same arguments, same order, in the same loop.
    """
    rng = rng or np.random.default_rng(0)
