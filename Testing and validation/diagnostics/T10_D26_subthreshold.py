...
rna_files = list((REPO / "architecture/output").glob("rna*.parquet"))
print(f"[COUNT] RNA output files found: {len(rna_files)}")
for f in rna_files:
    print(f"  {f.name}")

# Load the rna_scores output
rna_path = REPO / "architecture/output/rna_scores.parquet"
if not rna_path.exists():
    rna_path = REPO / "architecture/output"
...
output_dir = REPO / "architecture/output"
